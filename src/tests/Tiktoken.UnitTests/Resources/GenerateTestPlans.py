#!/usr/bin/env python3
"""
Generates TestPlans.txt with ground-truth token IDs from OpenAI's tiktoken library.

Usage:
    pip install tiktoken
    python GenerateTestPlans.py

Produces TestPlans.txt in the same directory with format:
    EncodingName: <encoding>
    Sample: <text>
    Encoded: [token1, token2, ...]
"""
...
def generate(out):
    for encoding_name in ENCODINGS:
        ...
        for sample in SAMPLES:
            tokens = enc.encode(sample, allowed_special=set())
            token_str = "[" + ", ".join(str(t) for t in tokens) + "]"
            out.write(f"EncodingName: {encoding_name}\n")
            out.write(f"Sample: {sample}\n")
            out.write(f"Encoded: {token_str}\n")
            out.write("\n")


def main():
    # --stdout flag: write to stdout instead of file (used by cross-validation test)
    if "--stdout" in sys.argv:
        generate(sys.stdout)
        return

    script_dir = os.path.dirname(os.path.abspath(__file__))
    output_path = os.path.join(script_dir, "TestPlans.txt")

    with open(output_path, "w", encoding="utf-8") as f:
        generate(f)

    total = len(ENCODINGS) * len(SAMPLES)
    print(f"Generated {total} test cases ({len(SAMPLES)} samples x {len(ENCODINGS)} encodings)")
